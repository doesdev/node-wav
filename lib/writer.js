...
const RIFF = bufferFrom('RIFF')
const WAVE = bufferFrom('WAVE')
const fmt = bufferFrom('fmt ')
const data = bufferFrom('data')
...
Writer.prototype._writeHeader = function () {
  ...
  const header = bufferAlloc(headerLength)
  let offset = 0
...
Writer.prototype._flush = function (done) {
  ...
  const emitHeader = this._emitHeader.bind(this)

  if (this.file) {
    this.file.on('close', emitHeader)
  } else {
    process.nextTick(emitHeader)
  }
}
