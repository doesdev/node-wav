...
Reader.prototype._onRiffID = function (chunk) {
  const id = this.riffId = chunk.toString('ascii')
  if (id === 'RIFF') {
    this.endianness = 'LE'
...
Reader.prototype._onSubchunk1ID = function (chunk) {
  const subchunk1ID = chunk.toString('ascii')
  this.chunkId = subchunk1ID
  if (subchunk1ID === 'fmt ') {
    this._bytes(4, this._onSubchunk1Size)
  } else if (subchunk1ID === 'JUNK') {
    this._bytes(4, this._skipBytes)
  } else {
...
Reader.prototype._onSubchunk2ID = function (chunk) {
  const subchunk2ID = chunk.toString('ascii')
...
Reader.prototype._onDataChunkSize = function (chunk) {
  let chunkSize = chunk['readUInt32' + this.endianness](0)
...
Reader.prototype._onFactChunkSize = function (chunk) {
  const chunkDataSize = chunk['readUInt32' + this.endianness](0)
...
Reader.prototype._onFactChunkData = function (chunk) {
  ...
  const numSamples = chunk['readUInt32' + this.endianness](0)
...
Reader.prototype._onUnknownChunkSize = function (chunk) {
  const chunkSize = chunk['readUInt32' + this.endianness](0)
...
Reader.prototype._skipBytes = function (chunk) {
  const skipSize = chunk['readUInt32' + this.endianness](0)
  this._bytes(skipSize, function () {
    this._bytes(4, this._onSubchunk1ID)
  })
}
